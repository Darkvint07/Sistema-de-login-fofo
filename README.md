...
- **Outros:** `jakarta.mail-2.0.2.jar` (reservado para recursos de e-mail em testes futuros).
...
1. Instale um servidor com PHP e MySQL (por exemplo, XAMPP).
2. Crie o banco de dados e a(s) tabela(s) de usuários conforme o script SQL que você utiliza (ex.: tabela `usuarios` com campos como `id`, `nome`, `email`, `senha`).
3. Ajuste host, usuário, senha e nome do banco em `conexao.php` e demais arquivos PHP que acessam o banco.
4. Copie os arquivos do projeto para a pasta do servidor (`htdocs` ou equivalente).
5. Acesse pelo navegador, por exemplo: `http://localhost/Sistema-de-login-fofo/form_login.html`.

### 2. Aplicação Java (desktop)

1. Importe o projeto Java em uma IDE (NetBeans, Eclipse, IntelliJ, etc.).
2. Adicione o `mysql-connector-j-9.4.0.jar` ao classpath do projeto.
3. Ajuste, nas classes de conexão, os mesmos dados do seu banco local.
4. Execute as classes de tela, como `TelaLogin.java` ou `SistemaLogin.java`, para testar o login/cadastro via Java.
...
- Explorar e **adaptar códigos prontos de tutoriais e IA**, aprendendo a ler, modificar e integrar diferentes partes de um sistema.
- Ganhar familiaridade com a organização de um projeto que mistura várias linguagens e camadas (frontend, backend e banco).

## Limitações e avisos

- Este sistema não é seguro para uso em produção: não implementa todas as boas práticas de segurança (hash de senha robusto, validação completa de entrada, prevenção de SQL Injection, etc.).
- O foco do projeto é estudo e portfólio de estágio, mostrando contato real com Java, PHP, HTML/CSS/JS e MySQL com apoio de materiais externos
